...
FORMULA_RULES = [
    (("implied returns and cash flow additivity",), "Cash flow additivity", "PV(CF_A + CF_B) = PV(CF_A) + PV(CF_B)", "Cash-flow streams with identical state-by-state payoffs must have the same present value; otherwise an arbitrage is possible.", "A four-year 100 annuity plus a 300 payment in year 3 replicates payments of 100, 100, 400, and 100, so their present values add."),
    (("probability models for portfolio return and risk",), "Portfolio variance", "σₚ² = ΣᵢΣⱼ wᵢwⱼ Cov(Rᵢ,Rⱼ)", "Portfolio risk depends on asset weights, standalone variances, and every pairwise covariance.", "Correlation below +1 creates a diversification benefit relative to the weighted average of standalone volatility; it does not guarantee risk below each asset."),
    (("fixed-income cash flows and types",), "Periodic coupon cash flow", "Coupon = (annual coupon rate ÷ payments per year) × current principal", "The current principal and contract terms determine the scheduled coupon; amortization and contingency provisions can change later cash flows.", "A 5% annual coupon on 1,000 par paid semiannually produces two scheduled coupons of 25 before any principal adjustment."),
    ...
    (("cost of capital",), "Weighted-average cost of capital", "WACC = wₑrₑ + w_d r_d(1 − T)", "Use market-value weights and component costs consistent with the project's operating risk.", "With equal debt and equity weights, 10% equity cost, 6% debt cost, and a 25% tax rate, WACC is 7.25%."),
